...
if False:
    ...
    # store data
    f = open('/Users/marcelvangerven/Code/github/DRM/store.pckl', 'wb')
    ...
    # load data
    f = open('/Users/marcelvangerven/Code/github/DRM/store.pckl', 'rb')
...
train_iter = DRMIterator(stimulus=train_stimuli, stim_time=train_stim_time,
                         response=train_response, resp_time=train_resp_time, batch_size=32)

test_iter = DRMIterator(stimulus=test_stimuli, stim_time=test_stim_time,
...
drm_model = DRMModel(populations, ws, Wp, readout, gpu=-1)
